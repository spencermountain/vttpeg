...
const tmpVtt = (contents) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vttpeg-'))
...
Clean already
`
  const file = tmpVtt(clean)
  // no input needed - it should not prompt
  const out = run(['--interactive', '--normalize', file], '')
...
test('keeps dots in episode-style filenames', (t) => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vttpeg-'))
  const file = path.join(dir, 'My.Show.S01E01.vtt')
...
test('accepts a ./ path prefix', (t) => {
  ...
  const out = execFileSync('node', [bin, '--lint', './sub.vtt'], { cwd: dir, encoding: 'utf8' })
